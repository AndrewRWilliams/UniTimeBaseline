...
class UniTime(nn.Module):
    def __init__(self, args):
        ...
        dec_trans_layer = nn.TransformerEncoderLayer(
        ...
        self.dec_transformer = nn.TransformerEncoder(
            dec_trans_layer, num_layers=args.dec_trans_layer_num
        )

        self.dec_head = FlattenHead(
            fea_num=self.d_model * args.max_token_num,
    ...
    def forward(self, info, x_inp, mask):
        ...
        stdev = torch.sqrt(
            torch.sum(x_inp * x_inp, dim=1) / torch.sum(mask == 1, dim=1) + 1e-5
        )
        ...
        if x_token.shape[1] > self.max_token_num:
            raise ValueError(
                f"x_token length {x_token.shape[1]} exceeds max_token_num {self.max_token_num}"
            )

        if len(instruct) > 0:
            instruct_ids = self.tokenizer(instruct, return_tensors="pt").input_ids.to(
                x_inp.device
            )
            instruct_embed = self.backbone.transformer.wte(instruct_ids).repeat(
                x_token.shape[0], 1, 1
            )
            total_token_count = instruct_embed.shape[1] + x_token.shape[1]
            if total_token_count <= self.max_token_num:
                inputs_embeds = torch.cat((instruct_embed, x_token), dim=1)
            else:
                available_token_count = self.max_token_num - x_token.shape[1]
                if available_token_count > 0:
                    instruct_embed = instruct_embed[:, :available_token_count, :]
                    inputs_embeds = torch.cat((instruct_embed, x_token), dim=1)
                else:
        ...
        x_dec = torch.reshape(x_dec, (-1, n_vars, x_dec.shape[-2], x_dec.shape[-1]))
